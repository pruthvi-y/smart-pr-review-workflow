...
def _server_parameters() -> StdioServerParameters:
    ...
    env = {
        ...
        "GITHUB_MAX_FILES": os.getenv("GITHUB_MAX_FILES", "50"),
        "MCP_LOG_LEVEL": os.getenv("MCP_LOG_LEVEL", "WARNING"),
    }
    return StdioServerParameters(
        command=sys.executable,
        args=["-m", "smart_pr_review_workflow.mcp.server"],
        env=env,
    )


async def _call_tool(client: Client, tool_name: str, arguments: dict[str, Any]) -> Any:
...
class MCPGitHubClient:
    ...
    def get_pull_request_bundle(
        ...
        """Discover MCP tools and use one MCP session for the PR metadata + changed files."""
        async def _bundle() -> tuple[dict[str, Any], list[dict[str, Any]]]:
            async with Client(_server_parameters()) as client:
                tools = await client.list_tools()
                names = {tool.name for tool in tools.tools}
                required = {"get_pull_request", "list_pull_request_files"}
